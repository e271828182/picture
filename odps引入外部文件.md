odps处理zip文件
 ...
 ```python
import sys
sys.path.insert(0,'http://schedule@{env}inside.cheetah.alibaba-inc.com/scheduler/res?id=132986473')
 ```
...
import sys
import os
Pillow_archive_file = 'http://schedule@{env}inside.cheetah.alibaba-inc.com/scheduler/res?id=144899728'
...
```python
spec = importlib.util.spec_from_file_location("update_legao_dataset", 'http://schedule@{env}inside.cheetah.alibaba-inc.com/scheduler/res?id=135885419')
...
```python
exec(open('http://schedule@{env}inside.cheetah.alibaba-inc.com/scheduler/res?id=147645763').read())
```
...
import sys
import os
a='http://schedule@{env}inside.cheetah.alibaba-inc.com/scheduler/res?id=106809472' ##引用资源路径
